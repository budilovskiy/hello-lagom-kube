...
/**
  ...
  */
class HelloServiceImpl(persistentEntityRegistry: PersistentEntityRegistry)(implicit ec: ExecutionContext) extends HelloService {
  ...
  override def internalHello(id: String) = ServiceCall { _ =>
    // Look up the hello entity for the given ID.
    val ref = persistentEntityRegistry.refFor[HelloEntity](id)

    // Ask the entity the Hello command.
    ref.ask(Hello(id)).map(resp => s"$resp (internal)")
  }
